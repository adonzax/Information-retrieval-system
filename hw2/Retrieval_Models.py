# -*- coding: utf-8 -*-
"""
Retrieval models on the **unstemmed** Cranfield index.

Toggle which models you want to run inside `main()`.  Output TREC-style runs
go to `Files/Unstemmed/Results/`.

Note: D = 1400 for the Cranfield collection (vs. 84,678 for AP89).
"""
...
D = 1400  # number of documents in the Cranfield collection
# ---------------------------------------------------------------------------
...
def queryNums():
    """Return the original Cranfield query IDs in input order."""
    import re
    ...
    for chunk in raw:
        ...
        # Mirror the original: drop punctuation from the query id
...
# ---------------------------------------------------------------------------
# Choose ONE pickle naming convention by setting USE_PROXIMITY_PICKLES.
# - False -> termStats<i>.p / termVector<i>.p   (used for OkapiTF/BM25/LM)
# - True  -> termStats_Proximity<i>.p / ...     (used for proximity scoring)
# ---------------------------------------------------------------------------
USE_PROXIMITY_PICKLES = True
...
def main():
    ...
    for i, qNo in enumerate(qNums, start=1):
        ...
        if not (os.path.exists(ts_path) and os.path.exists(tv_path)):
            print("Skip query %s (missing pickles)" % qNo)
            continue
