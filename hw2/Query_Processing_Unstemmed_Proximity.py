# -*- coding: utf-8 -*-
"""
Same idea as Query_Processing.py but reads queries from `cranfield/cran.qry`
(treated as the proximity query model file) and pickles the resulting
termStats / termVector under names with a `_Proximity` suffix.

By default this uses the standard Cranfield query file because the original
homework's `ProximityQueryModel.txt` only existed for AP89.  If you have a
custom proximity query file you may set CRAN_PROXIMITY_QRY_FILE below.
"""
...
CRAN_PROXIMITY_QRY_FILE = os.path.join(os.path.dirname(__file__), "cranfield", "cran.qry")
...
def parseCranfieldQueries(path):
    ...
    for chunk in raw:
        ...
        qtext = sections[1].strip() if len(sections) >= 2 else ""
        yield qno, qtext
...
def queryMaker():
    ...
    for qno, text in parseCranfieldQueries(CRAN_PROXIMITY_QRY_FILE):
...
def getParameters(query, qNo, catalog, termMap, docMap):
    ...
    with open(os.path.join(INDEX_DIR, "Pickles", "termStats_Proximity%s.p" % qNo), "wb") as f:
        dill.dump(termStats, f)
    with open(os.path.join(INDEX_DIR, "Pickles", "termVector_Proximity%s.p" % qNo), "wb") as f:
...
def main():
    ...
    queries = queryMaker()
    print("Loaded %d Cranfield proximity queries" % len(queries))
    qIdx = 0
