# -*- coding: utf-8 -*-
"""
Stemmed (Porter2) + stop-word removed inverted index builder for the
Cranfield collection.

Reads `cranfield/cran.all.1400`, stems every token with Porter2, removes
stop-words, and produces files under `Files/Stemmed/`:

    Files/Stemmed/invertedFile<n>.txt
    Files/Stemmed/catalogFile<n>.txt
    Files/Stemmed/catalogFile.txt
    Files/Stemmed/Maps/termMap.txt
    Files/Stemmed/Maps/docMap.txt
    Files/Stemmed/Pickles/{termMap,docMap,docInfo}.p
"""
...
OUT_DIR        = os.path.join(os.path.dirname(__file__), "Files", "Stemmed")
# ---------------------------------------------------------------------------
...
def parseCranfieldDocs(path):
    ...
    for chunk in raw_docs:
        ...
        text = (section_map.get("T", "") + " " + section_map.get("W", "")).strip()
        yield docno, text
...
# ---------------------------------------------------------------------------
# Indexing helpers (identical structure to the unstemmed version)
# ---------------------------------------------------------------------------
...
def loadCatalog(termDict, fileName, invFileNo, catalogFile=None):
    ...
    for term in termDict:
        # Sort docs in this term's list by TF descending
        termDict[term] = OrderedDict(
            sorted(termDict[term].items(), key=lambda x: x[1].tf, reverse=True))
        offset = invFile.tell()
...
def mergeInvFiles():
    ...
    for term in list(catalog.terms.keys()):
        for fname in catalog.terms[term]:
            ...
            invList = loadInvList(catalog.terms[term][fname].offset,
